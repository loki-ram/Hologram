# ============================================================
# ENHANCED INTERFEROGRAM DENOISING MODEL - READY TO TRAIN
# ============================================================
...
class InterferogramDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Load noisy interferogram
        noisy_path = os.path.join(self.root, self.files[idx])
        noisy = plt.imread(noisy_path).astype(np.float32)
        
        # Normalize if needed (your data is already [0,1])
        if noisy.max() > 1.0:
            noisy /= 255.0

        # Load clean phase (correct pattern: interf_00000.png -> Phi_00000.npy)
...
class AugmentedSubset(Subset):
    """Wrapper to enable augmentation on subset"""
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Train Enhanced Interferogram Denoiser")
    ...
    print("="*70)
    print("INTERFEROGRAM DENOISING - TRAINING")
    print("="*70)
    ...
    # Save final model
    torch.save(model.state_dict(), "enhanced_interferogram_denoiser_final.pth")
    print("\n✓ All done! Model saved as 'enhanced_interferogram_denoiser_final.pth'")
    # Final training summary plots (saved to visuals_dir)
    try:
        save_path = os.path.join(visuals_dir, "training_summary.png")
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
